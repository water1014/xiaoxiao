...
一个**纯前端、离线可用**的移动端 PWA 个人养成工作台。单文件 HTML 已按计划拆分为清晰的目录结构，方便在 GitHub 上管理、用 PWA Builder / HBuilder 打包成 Android APK。
...
```
.
├── index.html              # 主入口（484 行：结构 + 动态样式容器 + 按序引入 css/js）
...
├── css/                    # 15 个样式分片（01→15 顺序引入，层叠顺序与原单文件一致）
...
├── js/                     # 18 个脚本分片（01→18 顺序引入，共享全局作用域）
...
└── archive/                # 历次验证截图与报告（约 51MB，仅供追溯，不纳入打包）
```
...
1. **按原文件顺序物理切分**，不按功能重排。CSS 层叠与 JS 执行都依赖先后次序，重排会静默改变优先级、造成难以定位的回退。每个分片文件头都标注了「来源：原 index.html 第 X–Y 行」，改样式请就地编辑对应分片。
...
- **PWA 三件套已补齐**：`manifest.webmanifest` + `sw.js` + `assets/images/` 三张图标（含 maskable）。旧版 HTML 一直在引用这些文件却从未创建，是此前「装不上 PWA」的唯一原因——现已修复并通过验证（SW `activated`、manifest 可解析、4 图标 + sw.js 全部 200、零报错）。
- **`sw.js` 运行时缓存**：导航走 network-first（离线回退），同源静态资源走 stale-while-revalidate，**不硬编码文件清单**——新增的 css/js 自动纳入缓存，无需回头改 SW。
- **打包 APK**：
  - *PWA Builder*：把整个目录（含 `index.html` / `css/` / `js/` / `manifest.webmanifest` / `sw.js` / `assets/`）整体上传或指向其 https 地址，生成可安装 APK。
...
| 建议 | 现状 | 位置 |
|------|------|------|
| 底部导航弹性 | ✅ 已实现 | `.bot-tab .bt-item{flex:1}` 弹性等分 + `--ease-spring` 弹簧缓动 + 选中 `scale(1.08)` + 指示条弹簧滑动（`css/12-mobile-tabs.css`） |
| 打字光标主题色 | ✅ 已实现 | 首页问候语「打字机光标」呼吸光标（`css/14-patches.css`） |
| 卡片入场延迟 | ✅ 已实现 | `.stagger-item` 配 `--i` 变量做 `animation-delay:calc(var(--i)*.05s)` 错峰入场（`css/03-material.css`） |
...
| 项 | 结果 |
|----|------|
| 全部 css/js 无 404 | ✅ 本地 http 探活全 200 |
| 加载顺序正确（base→…→tail） | ✅ 15 CSS、18 JS 按序 `<link>`/`<script>` |
...
- **改样式**：定位对应 `css/NN-*.css` 就地修改，保持引入顺序不变。
- **改逻辑**：定位对应 `js/NN-*.js`；新增函数会自动挂到全局作用域（普通脚本），可被内联 `onclick` 直接调用。
- **新增资源**：放进 `assets/`，`sw.js` 会自动缓存；JS/CSS 直接放对应目录并按需加 `<script>`/`<link>` 即可。
